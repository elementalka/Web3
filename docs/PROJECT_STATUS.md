...
| CI and dependencies | Ready | Partial | Clean install, typecheck, 25 tests, build and dependency audit run in GitHub Actions. Production adds SAST, secret scanning and approval gates. |
| Vercel deployment | Ready | Blocked | One project serves Vite and a catch-all Fastify Function. `/tmp` state can reset at any cold start, so it is showcase-only. |
